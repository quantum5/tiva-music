...
charmap = u'ĄĘĆŹŻąęćźżéêàâÄÖÜäöüŁłŃńŚśÓóèôî'
charmap = [(ch, u'\\x%02x""' % i) for i, ch in enumerate(charmap, 1)]
charmap.insert(0, ('"', r'\"'))
remove_quotes = re.compile(r'(?<!\\)\"\"(?![0-9a-fA-F])')
...
class MusicToC(Parser):
    ...
    def slur(self, new):
        super(MusicToC, self).slur(new)
        if not new and self.notes:
            freq, length = self.notes[-1]
            self.notes[-1] = (freq & ~0x8000, length)
    ...
    def say(self, line):
        ...
        self.lyrics.append((self.index, remove_quotes.sub('', line)))
    ...
    def play(self, note, length):
        if note.startswith('0'):
            ...
            self.notes.append((int(table[note]) | (self.slurred and 0x8000), length))
        self.index += 1

    def run(self):
        super(MusicToC, self).run()

        note_array = ', '.join('{%d, %d}' % (freq, length) for freq, length in self.notes)
        note_lyrics = ', '.join('{%d, "%s"}' % (index, line) for index, line in self.lyrics)
...
''')
        self.output.write(u'static const note song_notes[] = {%s};\n' % (note_array))
        if self.lyrics:
            self.output.write(u'static const lyric_line song_lyrics[%d] = {%s};\n' % (len(self.lyrics), note_lyrics))
        self.output.write(u'''
const sw_song %s = {
    song_notes, ARRAY_SIZE(song_notes),
''' % self.name.replace('-', '_').replace(' ', '_'))
        if self.lyrics:
            self.output.write(u'    song_lyrics, ARRAY_SIZE(song_lyrics),\n')
        else:
            self.output.write(u'    0, 0,\n')
        self.output.write(u'};\n')
...
def main():
    ...
    for file in sys.argv[1:]:
        try:
            ...
            with io.open(file, encoding='utf-8') as input, \
                 io.open(basename + '.c', 'w', encoding='utf-8') as output:
                MusicToC(input, output, os.path.splitext(basename)[0]).run()
        except IOError as e:
            print e.strerror, 'on opening:', e.filename
